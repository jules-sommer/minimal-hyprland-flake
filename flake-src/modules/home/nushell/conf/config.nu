...
let extensions = {
  ...
  image: ["png", "jpg", "jpeg", "webp", "gif", "tif", "bmp", "svg", "ico"],
  video: ["mp4", "m4v", "mkv", "webm", "mov", "avi", "wmv", "mpg", "flv"],
  audio: ["mp3", "wav", "aac", "ogg", "flac", "m4a", "wma"],
  document: ["pdf", "doc", "docx", "odt", "txt", "rtf", "tex", "ppt", "pptx", "xls", "xlsx", "ods"],
  code: ["html", "css", "js", "jsx", "ts", "tsx", "json", "xml", "yaml", "yml", "md", "py", "rb", "java", "c", "cpp", "h", "hpp", "go", "rs", "swift"],
  database: ["sql", "db", "dbf", "mdb", "accdb", "sqlite", "csv"],
};

def get_extension [type: string, --string] {
  let ext_list = ($extensions | get $type);
  if ($ext_list | is-empty) {
    error make {
...
def find [
  ...
  if $type == null { ls; return; }

  let ext_matches = get_extension $type --string;
  if ($ext_matches == null) {
    let span = (metadata $type).span;
    print ($extensions | columns);
    error make {
      msg: $"Type must be one of the following: ($extensions | columns)",
      label: {
        text: $"You passed in ($type)",
        span: $span
      }
    }
  }
  ...
  if ($path | path exists) == false {
    error make {
      ...
      label: {
        text: $"You passed in ($path)",
        span: (metadata $path).span
      }
    }
  }

  let regex = $"\(?i\).\(($ext_matches | str join '|')\)$"
  print $ext_matches;
  print $regex;
  
  ls $path | where name =~ ($regex)
}
